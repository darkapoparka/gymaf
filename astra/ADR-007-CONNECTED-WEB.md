...
This is the strict server-only alternative contemplated in the earlier architecture, not the Supabase browser/SSR SDK pattern. It avoids adding an unverified SDK dependency/lockfile change but adds responsibility for provider-contract compatibility, refresh failure behavior, cookie/CSRF tests, MFA lifecycle and account recovery. Native bearer clients have an API boundary, but no native application has been implemented. Social login is not implemented.

Local review amendment, 6 September 2026: `GYMAF_EMAIL_AUTH_MODE=link` supports hosted Supabase's default confirmation/magic-link templates. Requests use an S256 PKCE challenge and an hour-lived HttpOnly verifier cookie. `/auth/callback` exchanges the single-use authorization code server-side, verifies the provider identity and registers the application session before setting existing session cookies. Redirects use the configured origin and fixed paths; callback query strings are excluded from Next development request logs. The newest link must be opened on the requesting browser/device. The default code mode remains for disposable local Supabase with the supplied OTP templates. Hosted invitations keep their original tab/token in memory while login runs in another tab. Actual hosted callback completion remains a separate provider acceptance check.
