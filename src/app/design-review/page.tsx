...
export default async function Page({searchParams}:{searchParams:Promise<{view?:string;width?:string;frame?:string}>}) {
 ...
 const q=await searchParams, views=['home','library','summary','progress','schedule','profile','workout','player','settings','location','equipment','injury','event','weight','log-weight','app-settings','workout-settings','tones','instructions'];
