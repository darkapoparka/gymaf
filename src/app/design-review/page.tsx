...
export default async function Page({searchParams}:{searchParams:Promise<{view?:string;width?:string;frame?:string}>}) {
 ...
 const q=await searchParams, views=['profile-edit','profile-error','photos','photo-error','cover-photos','avatar-photos','home','library','summary','feedback','progress','schedule','profile','workout','player','settings','location','equipment','injury','event','weight','log-weight','app-settings','workout-settings','tones','instructions'];
