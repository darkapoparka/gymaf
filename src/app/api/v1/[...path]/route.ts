...
export async function GET(request: NextRequest, context: Context) {
  try {
    ...
    if (kind === "export") result = { ...(result as Record<string, unknown>), coachRatings: await rpc('gymaf_coach_rating_query',{p_id:null},token), profileInterests: await rpc('gymaf_interests_query',{},token), memberRecords: await rpc("gymaf_member_query", {}, token), workoutFavorites: await rpc("gymaf_training_query", { p_kind: "export-favorites", p_id: null }, token), sessionFeedback: await rpc("gymaf_feedback_query", { p_id: null }, token), media: await rpc('gymaf_media_query',{},token) };
