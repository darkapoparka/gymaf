...
export async function POST(request: NextRequest, context: Context) {
  try {
    ...
    if (!selecting && path.join('/') !== 'me/attachments' && path.join('/') !== 'me/friends/invitation' && path.join("/") !== "commands" && path.join('/') !== 'me/media') throw new HttpError(404, "NOT_FOUND", "Unknown command endpoint.");
