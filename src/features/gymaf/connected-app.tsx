...
export function ConnectedApp({ area, path = [], relationshipId, workspaceId }: { area: "app" | "coach" | "operator"; path?: string[]; relationshipId?: string; workspaceId?: string }) {
  ...
  return <div className="gymaf-connected" lang={data.user.locale}><header className="gymaf-topline"><Link href="/" className="wordmark">Gymaf</Link><div className="gymaf-account-links"><Link href="/app">Client</Link>{data.workspaces.length>0 && <Link href="/coach">Coach</Link>}{data.operator && <Link href="/operator">Operator</Link>}<Link href="/app/profile">Account</Link></div></header>
    <p className="gymaf-banner">{data.local_mode ? "Synthetic local environment · no live payments · MFA bypass is local-only" : "Pre-release integration build · not approved for real client data or public launch"}</p>
    <a href="#main" className="skip-link">Skip to content</a>
    <main id="main" className={`app-shell ${area === "app" && !path.length ? "home-page" : ""}`}>
    ...
    {area !== "operator" && !sessionId && <Navigation area={area} active={path[0] || ""} locale={data.user.locale} query={query} />}
  </div>;
}
