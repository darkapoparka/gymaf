...
export function Messages({ relationshipId, userId, canSend, coachName = "Your coach", presentation = "embedded", initialComposer=false }: { relationshipId: string; userId: string; canSend: boolean; coachName?:string; presentation?: "client" | "embedded" | "template";initialComposer?:boolean }) {
  ...
  if (presentation === 'template') return <section className="messages-page template-messages">
    <header className="chat-header"><Avatar size={23}/><h1>{coachName}</h1><Link className="icon-button" aria-label="Shared media" href={`/app/messages/shared?relationship=${relationshipId}`}><Images size={23}/></Link></header>
    <div className="chat-messages" role="log" aria-label="Coach conversation"><ErrorNote message={error || mutation.error || attachments.error}/>{error && <button className="button" onClick={() => void load().catch(() => setError('Refresh failed. Please retry.'))}>Refresh conversation</button>}{data?.next_cursor && <button className="button" disabled={loadingOlder} onClick={() => void older()}>Load older messages</button>}{!data ? <p className="note">Loading conversation…</p> : !data.messages.length ? <p className="note">Your conversation starts here.</p> : [...data.messages].reverse().map(message => <article key={message.id}><div className={message.sender_id === userId ? 'outgoing-message' : 'coach-message'}><p>{message.body}</p>{attachments.data?.filter(item => item.message_id === message.id).map(item => <AttachmentPreview key={item.id} item={item}/>)}</div><p className="chat-timestamp">{new Date(message.created_at).toLocaleString()}{message.sender_id === userId ? data.other_read_at && Date.parse(data.other_read_at) >= Date.parse(message.created_at) ? ' · Read' : ' · Sent' : ''}</p></article>)}<Link ref={ratingTrigger} className="coach-rating-link" href={`/app/messages/rate?relationship=${relationshipId}`}>How is it going with {coachName}?</Link></div>
    {canSend ? <form className="message-composer" onSubmit={event => {event.preventDefault();void send();}}><button type="button" className="icon-button" aria-label="Add photo or video" disabled={mutation.busy} onClick={() => setAdding(true)}><Plus/></button><label><span className="sr-only">Message to your coach</span><input placeholder="Send a message" maxLength={4000} value={body} disabled={mutation.busy} onChange={event => setBody(event.target.value)}/><button type="submit" aria-label="Send message" disabled={mutation.busy || !body.trim()}><ArrowUp size={20}/></button></label></form> : <p className="note">This relationship is read-only.</p>}
    {adding && canSend && <div className="gymaf-connected gymaf-client"><AttachmentComposer ownerId={userId} relationshipId={relationshipId} coachName={coachName} onClose={() => setAdding(false)} onShared={() => {attachments.reload();void load().catch(() => setError('Media was sent. Refresh the conversation to see it.'));}}/></div>}
  </section>;
