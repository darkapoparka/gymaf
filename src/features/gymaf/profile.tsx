...
export function ProfileScreen({ account, relationship, reload }: { account: Bootstrap; relationship?: RelationshipDetail; reload: () => void }) {
  ...
  const mutation = useCommand(), requests = useResource<DataRequest[]>("me/requests");
  ...
  return <div className="gymaf-stack"><Head title={u.locale === "bg" ? "Профил" : "Profile"} /><Note>Account ID: <code>{u.id}</code></Note>
    ...
    <MfaSettings onVerified={reload} />
    {relationship && <section className="gymaf-panel gymaf-stack"><h2>Coaching service</h2><p>{relationship.relationship.coach_name || "Your coach"} · {relationship.relationship.state}</p>{relationship.entitlements.map(e => <p key={e.id}>{e.source} · {e.state} · {new Date(e.ends_at).toLocaleDateString()}</p>)}
    ...
    <section className="gymaf-panel gymaf-stack"><h2>Support and your data</h2><button className="button" onClick={() => void download()}>Download my data</button>
    ...
    <ErrorNote message={mutation.error || externalError} /><Note>{notice}</Note><button className="button full" onClick={() => void logout()}>Sign out</button>
  </div>;
}
