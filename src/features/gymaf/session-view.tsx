...
import { Check, ChevronLeft, ChevronRight, History, Flag, List, Pause, Play, X, Video } from 'lucide-react';
...
type Props = {
  ...
  renderRecording?: (exerciseId: string, close: () => void) => ReactNode;
...
export function SessionView({ detail, elapsed, back, dirty, busy, error, artwork, preferences = {}, onTransition, renderSets, renderHistory, renderFlag, renderRecording }: Props) {
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{
      ...
      else {setCountdown(null);setIndex(i=>Math.min(i+1,detail.session.prescription.exercises.length-1));}
    },1000);
    return()=>clearTimeout(timer);
  },[countdown,detail.session.prescription.exercises.length]);
  function nextExercise(){
    if(preferences.vibration&&'vibrate' in navigator)navigator.vibrate(80);
    // Audio is created only within this user gesture; blocked audio never blocks training.
    try {
      const context=audio.current||(audio.current=new AudioContext());
      void context.resume().catch(()=>undefined);
      const oscillator=context.createOscillator(),gain=context.createGain();
      oscillator.type=preferences.tone==='Beep'?'sine':'triangle';
      oscillator.frequency.value=preferences.tone==='Beep'?880:523.25;
  ...
  const locked = dirty || busy || countdown!==null;
  ...
  return <section className={`connected-player ${paused ? 'is-paused' : ''}`}>
    ...
    <div className="connected-player-controls">
      ...
      <div className="player-exercise-nav">
        ...
        <button className="player-log-sets" disabled={busy||countdown!==null} aria-label={editable ? "Log Sets" : "View Sets"} onClick={() => setSheet("sets")}><span>{logged}/{exercise.sets.length} sets logged</span><small>Exercise {index + 1} of {exercises.length} · {editable ? "Log Sets" : "View Sets"}</small></button>
        <button className="icon-button" disabled={locked || index === exercises.length - 1} onClick={nextExercise} aria-label="Next exercise"><ChevronRight/></button>
      </div>
      {expanded && <div className="player-action-row">
        ...
        {editable&&renderRecording&&<button className="button" disabled={locked} onClick={()=>setRecording(true)}><Video size={18}/>Record</button>}
    ...
    {recording && renderRecording?.(exercise.id,()=>setRecording(false))}
