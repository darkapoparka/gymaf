...
export function CoachArea({ account, path, workspaceId, reloadAccount }: { account: Bootstrap; path: string[]; workspaceId?: string; reloadAccount: () => void }) {
  ...
  if (path[0] === "profile") return <div className="gymaf-stack"><WorkspaceProfile data={data} reload={resource.reload} /><DirectoryEditor ownerId={account.user.id} workspaceId={selected.id}/><ProfileScreen key={account.user.revision} account={account} reload={reloadAccount} /></div>;
