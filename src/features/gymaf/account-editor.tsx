...
function AccountEditorView({user,query,data}:{user:Profile;query:string;data:AccountResource}) {
  ...
  return <section className="connected-profile-editor connected-account-editor" aria-busy={busy}>
    ...
    </fieldset></form><div className="account-footer"><Link href={`/app/account/shipping${query}`}>Shipping Address <ArrowRight/></Link><SignOutButton/><Link href={`/app/settings/service${query}`}>Membership &amp; Billing <ArrowRight/></Link><Link href={`/app/settings/support${query}`}>Account Help <ArrowRight/></Link></div>
