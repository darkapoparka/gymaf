...
export type ProfileFields = { displayName:string; locale:Profile['locale']; timezone:string; goal:string; equipment:string; availability:string };
export type ProfileDraft = { fields:ProfileFields; baseline:ProfileFields; revision:number; commandId?:string };
const drafts = new Map<string, ProfileDraft>();
export function profileFields(user:Profile):ProfileFields {
  return {displayName:user.display_name,locale:user.locale,timezone:user.timezone,goal:user.goal,equipment:user.equipment,availability:user.availability};
}
...
export function profileChanged(draft:ProfileDraft) { return (Object.keys(draft.fields) as (keyof ProfileFields)[]).some(key=>draft.fields[key]!==draft.baseline[key]); }
