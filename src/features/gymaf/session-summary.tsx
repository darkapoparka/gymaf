...
export function SessionSummary({detail,back,renderSets}:{detail:SessionDetail;back:string;renderSets:(id:string)=>ReactNode}) {
  const [tab,setTab]=useState('Summary'),[exerciseId,setExerciseId]=useState(''),[template,setTemplate]=useState(0);
  ...
  return <section className="connected-session-summary"><header className="native-titlebar"><Link href={back} className="icon-button" aria-label="Close Summary"><X/></Link><h1>{date.toLocaleDateString(undefined,{month:'short',day:'numeric'})} · {date.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'})}</h1><span/></header>
    <div className="tabs" role="group" aria-label="Workout summary">{['Summary','Exercises'].map(t=><button key={t} className={t===tab?'active':''} aria-pressed={t===tab} onClick={()=>setTab(t)}>{t}{t==='Exercises'&&<small>{session.prescription.exercises.length}</small>}</button>)}</div>
