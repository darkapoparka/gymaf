...
export function ProfileEditorView({user,query='',cover,footer,mediaError,retryMedia,onLeave,onSave,onReload}:{user:Profile;query?:string;cover?:ReactNode;footer?:ReactNode;mediaError?:string;retryMedia?:()=>void;onLeave:()=>void;onSave:(draft:ProfileDraft,signal:AbortSignal)=>Promise<void>;onReload:(signal:AbortSignal)=>Promise<Profile>}) {
  ...
  function change(key:keyof ProfileFields,value:string) { update({...draft,fields:{...draft.fields,[key]:value},commandId:undefined}); }
  ...
  async function save() {
    ...
    catch(failure) { if(mounted.current)setError(failure instanceof Error?failure.message:'Your profile could not be saved. Retry Save.'); }
  ...
  async function reloadSaved() {
    ...
    catch(failure) { if(mounted.current)setError(failure instanceof Error?failure.message:'Saved profile could not be loaded. Your edits are still here.'); }
    finally { running.current=false;if(mounted.current)setBusy(false); }
  }
  return <section className="connected-profile-editor" aria-busy={busy}>
    ...
    <form id={formId} ref={form} onSubmit={event=>{event.preventDefault();void save();}}>
      <fieldset disabled={busy}>
        ...
        <section className="profile-account-fields"><h2>Account Preferences</h2><label className="form-field"><span>Language</span><select value={draft.fields.locale} onChange={event=>change('locale',event.target.value)}><option value="bg">Български</option><option value="en">English</option></select></label><Field label="Timezone" required maxLength={80} value={draft.fields.timezone} onChange={event=>change('timezone',event.target.value)}/></section>
      ...
      <ErrorNote message={error}/>{error&&<p className="profile-field-help">Your edits are still here. Retry Save, or reload the saved profile to resolve a conflict.</p>}
    ...
    <div className="profile-editor-links"><Link href={`/app/settings/security${query}`}>Account Security</Link><Link href={`/app/settings/support${query}`}>Help &amp; Your Data</Link>{footer}</div>
    {dialog&&<Dialog title={dialog==='leave'?'Save profile changes?':'Reload saved profile?'} className="profile-exit-dialog" onClose={()=>{if(!running.current)setDialog(null);}}>
      <p>{dialog==='leave'?'You have unsaved changes to your profile.':'Reloading replaces your edits with the last saved profile. You can keep editing instead.'}</p><ErrorNote message={error}/>
